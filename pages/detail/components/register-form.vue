...
export default {
	...
	watch: {
		'formData.bounghtDate'(value) {
			...
			const newDate = new Date(newTime),
				...
				day = newDate.getDate();
			this.formData.expireDate = `${year}-${month.toString().length == 1 ? '0' : ''}${month}-${day.toString().length == 1 ? '0' : ''}${day}`;
			console.log(value);
		}
	},
	mounted: function() {},
	methods: {
		...
		handleBounghtDateChange: function(e) {
			this.formData.bounghtDate = e.target.value;
		}
	}
};
</script>
